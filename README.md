...
```bash
SAR_tutorial$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
SAR_tutorial$ sam local start-api
SAR_tutorial$ curl http://localhost:3000/
```
...
```bash
SAR_tutorial$ sam logs -n HelloWorldFunction --stack-name SAR_tutorial --tail
```
...
```bash
aws cloudformation delete-stack --stack-name SAR_tutorial
```
